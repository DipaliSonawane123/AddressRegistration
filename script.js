...
function insertNewRecord(data) {
    var table = document.getElementById("addressList").getElementsByTagName('tbody')[0];
    var newRow = table.insertRow(table.length);
    cell1 = newRow.insertCell(0);
    cell1.innerHTML = data.fullName;
    cell2 = newRow.insertCell(1);
    cell2.innerHTML = data.landmark;
...
function onEdit(td) {
    ...
    document.getElementById("landmark").value = selectedRow.cells[1].innerHTML;
...
function updateRecord(formData) {
    ...
    selectedRow.cells[1].innerHTML = formData.landmark;
...
function onDelete(td) {
    if (confirm('Are you sure to delete this record ?')) {
        ...
        document.getElementById("addressList").deleteRow(row.rowIndex);
        resetForm();
    }
}
function validate() {
